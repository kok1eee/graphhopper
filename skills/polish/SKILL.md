...
const LENSES = [
  { name: 'requirement', charter: 'あなたは要件逸脱の敵対的レビュアー。実装がrequirements/design.mdの意図から外れていないか（実装漏れ・解釈ズレ・スコープ逸脱）を検証する。' },
  { name: 'behavior', charter: 'あなたは挙動の敵対的レビュアー。テストが実際のユーザーパスを検証しているか（モック過多・ハッピーパスのみ・エラー握り潰し）を疑う。' },
  { name: 'progress', charter: 'あなたは進捗の敵対的レビュアー。変更がgoalに収束しているか（堂々巡り・残骸・goal無関係な混入）を見る。' },
]
...
// researcher=haiku, advisor/verifier=opus。opus指定を1つでも落とすと保証が silent に消えるので
// スクリプト編集時は必ず全agent()呼び出しにmodel:'opus'があることを確認する。
phase('Verify')
...
無ければ findings: [] と summary に「drift なし」と書いてください。`,
    { model: 'opus', label: 'verify:' + l.name, phase: 'Verify', schema: FINDING_SCHEMA })
...
- **`model: 'opus'` を1つでも欠かさない**: 3レンズは全て判断が要るnode（tiering:
  main=sonnet/researcher=haiku/advisor・verifier=opus）。編集時にレンズを増やすなら
  必ず新しい `agent()` 呼び出しにも `model: 'opus'` を明示する（ultraworkと同じ規律。
  1箇所でも落とすとテストでは検出できず「常にopus品質」の保証が silent に消える）。
